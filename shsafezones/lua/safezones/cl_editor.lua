...
function SH_SZ:MakePoint(parent, pos, mx, my)
	...
	pnt.DoMove = function(me)
		...
		local t = {
			...
			filter = self.HitProps and player.GetAll() or ents.GetAll(),
		}
...
function SH_SZ:MakeTabAdd(body)
	...
	tab_add:Dock(FILL)
		...
		self:PaintScroll(opts)
			...
			AddOption("noveh", "delete_vehicles")
			AddOption("ptime", "time_until_protection_enables")
			AddOption("pusg", "usergroups_with_protection")
...
hook.Add("CreateMove", "SH_SZ.CreateMove", function(cmd)
	if (!_SZ_EDITOR_OPEN or gui.IsGameUIVisible()) then
		return end
	
	local foc = vgui.GetKeyboardFocus()
	if (IsValid(foc) and foc:GetName() == "DTextEntry") then
		return end
