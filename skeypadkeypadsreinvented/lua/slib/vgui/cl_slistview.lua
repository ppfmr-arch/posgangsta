...
local textcolor, maincolor_7, linecol, neutralcolor, margin = slib.getTheme("textcolor"), slib.getTheme("maincolor", 7), Color(0,0,0,160), slib.getTheme("neutralcolor"), slib.getTheme("margin")
...
function PANEL:addLine(...)
    ...
    self.Lines[iteration].Paint = function(s,w,h)
        ...
        for i = 1,#self.Columns do
            ...
            if w < (self:getTextWidth(display) + margin * 2) then self:resizeColumns() end
            if i == 1 then
                s.name = display
            end

            if self.Columns[i].ignoreWidth then
